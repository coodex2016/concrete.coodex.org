...
public class AMQPSyncInvokerFactory implements InvokerFactory {
    @Override
    public Invoker getInvoker(Destination destination) {
        return new AMQPSyncInvoker(destination, new AMQPInvoker((AMQPDestination) destination));
    }

    @Override
    public boolean accept(Destination param) {
        return param != null && !param.isAsync() && param instanceof AMQPDestination;
    }
}
