 ...
 *
 * @param <T> The {@link net.sf.marineapi.provider.event.ProviderEvent} to be dispatched.
...
public abstract class AbstractProvider<T extends ProviderEvent> implements
    ...
    private void expunge() {
        ...
        List<SentenceEvent> expired = new ArrayList<>();
        for (SentenceEvent e : events) {
            if (now - e.getTimeStamp() > this.timeout) {
                expired.add(e);
            }
        }
